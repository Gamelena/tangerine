...
class Components_DojoChartController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        // Si existe $this->_layout[0]['TARGET'] se usa el modelo especificado
        // en el XML,
        ...
        if (! empty($uri)) {
            ...
            if (! empty($aUri)) {
                ...
                if ($this->_xml->getAttribute("target")) {
                    $aParams['model'] = $this->_xml->getAttribute("target");
                } else {
                    if ($this->_request->getParam("target")) {
                        $aParams['model'] = $this->_request->getParam("target");
                    }
                }
                
                if ($this->_xml->getAttribute("group_by")) {
                    $aParams['group'] = $this->_xml->getAttribute("group_by");
                } else {
                    if ($this->_request->getParam("group_by")) {
                        $aParams['group'] = $this->_request->getParam("group_by");
                    }
                }
    ...
    public function excelAction()
    {
        ...
        $this->view->content = chr(255) . chr(254) .
                 mb_convert_encoding($content, 'UCS-2LE', 'UTF-8');
        $this->render();
    }
}
