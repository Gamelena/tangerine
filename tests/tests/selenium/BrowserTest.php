...
class BrowserTest extends PHPUnit_Extensions_Selenium2TestCase
{
    ...
    public function iterateTree($tree)
    {
        $openLater = array();
            ...
            try {
                if ($branch['parent_id'] && !$this->byId('dijitEditorMenuModule'.$branch['id'])->displayed()) {
                    $itemParent = $this->byId('dijitEditorMenuModule'.$branch['parent_id']);
                    $itemParent->click();
                }
                
                $item = $this->byId('dijitEditorMenuModule'.$branch['id']);
                $item->click();
                sleep(PHPUNIT_WAITSECONDS/3);
