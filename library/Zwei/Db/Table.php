 ...
 * @since 0.1
 */
class Zwei_Db_Table extends Zend_Db_Table_Abstract /*implements 
        Zwei_Admin_ModelInterface*/
{

    /**
     * Campo etiqueta o título, util cuando se usa el modelo para graficar en 2
     * dimensiones
     * ya que especifica cual es el índice de las etiquetas del eje X.
     *
     * (*) Es obligatorio inicializar $this->_label o $this->_labels para
     * graficar en 2D.
     *
     * @var string | false
     */
    ...
    /**
     * Array de etiquetas o títulos, util cuando se usa el modelo para graficar
     * en 2 dimensiones.
     ...
     * @var boolean | array
     */
    protected static $_defaultLogMode = false;

    /**
     * Campos sobre los cuales se puede realizar la búsqueda por defecto, esto
     * tambien se puede hacer vía XML
     * 
     * @var array
     */
    ...
    /**
     * Devuelve respuesta para ejecutar javascript segun valor en
     * Zwei_Admin_Components_Helpers_EditTableDojo().
     * 
     * @var string
     */
    ...
    /**
     * indica si tendra filtros personalizados, ignorando los filtros de
     * Zwei_Db_Object para metodo select.
     * 
     * @var boolean
     */
    ...
    /**
     * Se asocia un título al recordset modelo, el cual puede ir en metadata de
     * archivo json.
     *
     * @var string
     */
    ...
    /**
     * Variable auxiliar para retornar cualquier dato(s) que necesitemos durante
     * la operación de un modelo.
     * Si está inicializada, esta variable es retornada como parte de la
     * respuesta Json en Zwei_Admin_Components_Helpers_EditTableDojo.
     * 
     * @var array|false
     */
    ...
    /**
     * Hash que indica si deben ser validados los permisos
    ...
    /**
     * __call() es lanzado al invocar un método inaccesible en un contexto de
     * objeto.
     * 
     * @link http://php.net/manual/es/language.oop5.overloading.php
     *      
     *       Permite usar self::findBy$field($value) sin tener que escribir los
     *       finders manualmente.
     *      
     *       Por defecto convierte $field de UpperCamelCase a Underscore. Ej:
     *       'IdClasePlataforma' ==> 'id_clase_plataforma'
     *      
     * @param string $function            
     * @param string $args[0]
     *            - $value (valor para $field)
     * @param string $args[1]
     *            - si es true, NO hace conversion de UpperCamelCase a
     *            undercore.
     */
    ...
    /**
     * Configura el adaptador de base de datos segun valores de atributo de
     * configuración.
    ...
    {
        $resource = Zend_Controller_Front::getInstance()->getParam("bootstrap")->getResource(
                "multidb");
    ...
    public function insert (array $data)
    {
        if (class_exists("SettingsModel")) {
            ...
            $logMessage = "[$userName $ip] INSERT INTO " .
                     $this->info(Zend_Db_Table::NAME) . " VALUES ($strData) ";
            Debug::writeBySettings($logMessage, 'transactions_log', '1', 
                    "../log/transactions");
        } else {
     ...
     * @param
     *            array array columna valor para forzar guardar tales valores en
     *            tales columnas.
        ...
        if ($update && class_exists("SettingsModel")) {
            if ($rowOrig) {
                try {
                    ...
                    if ($rowNew) {
                        $differences = Zwei_Utils_Array::getDifferences(
                                $rowOrig->toArray(), $rowNew->toArray());
                        $differences = print_r($differences, true);
                    } else {
                        Debug::write(
                                'FIXME, modificaron mi identidad y perdí el seguimiento');
                    }
                } catch (Zend_Db_Exception $e) {
                    Debug::write($e->getCode() . " " . $e->getMessage());
                }
            ...
            $logMessage = "[$userName $ip] UPDATE " .
                     $this->info(Zend_Db_Table::NAME) . " SET (" . $strData .
                     ") WHERE (" . $strWhere . ")";
            Debug::writeBySettings(
                    $logMessage . "\nValores Originales antes de Modificar : " .
                             $differences, 'transactions_log', '1', 
                            "../log/transactions");
        }
    ...
    public function delete ($where)
    {
        if (class_exists("SettingsModel")) {
            ...
            $strWhere = print_r($where, true);
            $logMessage = "[$userName $ip] DELETE FROM " .
                     $this->info(Zend_Db_Table::NAME) . " WHERE ($strWhere) ";
            // Debug::writeBySettings($this->getAdapter()->getConfig(),
            // 'transactions_log', '1', "../log/transactions");
            Debug::writeBySettings($logMessage, 'transactions_log', '1', 
                    "../log/transactions");
        } else {
            Debug::write('No existe SettingsModel');
    ...
    /**
     * Se setea attributo $this->_label el cual puede ser usado (por ejemplo)
     * para almacenes de datos js como dojo.data.ItemFileReadStore
     *
     * @param string $value            
     * @return void
     */
     ...
     * Lleva prefijo Zw para distinguirlo de método nativo
     ...
     * se ha realizado una operación CRUD, por ejemplo un mensaje personalizado
     * de error
     * 
    ...
    /**
     * Retorna el array de labels que etiquetan al recorset (ejemplo típico, un
     * intervalo de fechas).
     * 
     * @return array
     */
    ...
    /**
     * Agregar string javascript para añadir validación adicional para la
     * busqueda,
    ...
    /**
     * Cuentame más: retorno de información auxiliar que puede ser util despues
     * de ejecutar una operación crud.
     * 
     * @return array
     */
    ...
    /**
     * Activar o desactivar funcionalidad de usar parametros de búsqueda en XML
     * de forma automática
     * 
     * @param boolean $value            
     * @return void
    ...
    static public function whereToArray ($where)
    {
        ...
        foreach ($where as $string) {
            if (stristr($string, '=')) {
                ...
                $return[trim(str_replace("`", "", $array[0]))] = trim(
                        str_replace("'", "", $array[1]));
            }
        }
        return $return;
    ...
    /**
     * Valida los permisos de usuario en sesion en modelo $form->model para
     * ejecutar $form->action
     * según permisos sobre archivo xml $form->p.
     *
     * @param
     *            Zwei_Utils_Form<model, p, action> $form
    ...
    public function validateXmlAcl ($form, $xml = null)
    {
        ...
        if (! $xml) {
            // Si esto es llamado desde CrudRequestController SIEMPRE existe
            // $xml y nunca entra acá.
            if (isset($form->p)) {
        ...
        if ($xml->getAttribute('target') !== $form->model) {
            ...
            if (! $this->_acl->isUserAllowed($form->p, $action)) {
                $validatedList = false;
            }
    ...
    /**
     * Extiende arreglo de datos nativo de $self::select() para el despliegue en
     * listado.
     *
     * @param
     *            array - retorno de $self::select()
    ...
    /**
     * Extiende arreglo de datos nativo de $self::select() para el despliegue en
     * formulario cargado via ajax.
     *
     * @param
     *            Zend_Db_Rowset retorno de $self::select()
    ...
    /**
     * Indica si se guardarán transacciones en log, Modelo debe extender de
     * Zwei_Table_Loggeable()
     * 
     * @param
     *            boolean
     */
    ...
    /**
     * Método para separar datos de tabla principal de datos de tablas
     * auxiliares, para ser reescrito en modelos respectivos.
     *
    ...
    public function log ($action, $condition, $table = null)
    {
        ...
        if (self::$_defaultLogMode) {
            try {
                ...
                // Si se elimina un perfil con permisos asociados se genera esta
            // Exception
                // ya que aun no son borrados los permisos asociados, generando
            // una inconsistencia referencial,
                // estos permisos se borrarán despues, una forma de evitar esto
            // es borrando en cascada por ORM o DB.
            }
        }
    }
}
...
>>>>>>> 759d94a85e834c5410f90de9039fb5c321fe7557
