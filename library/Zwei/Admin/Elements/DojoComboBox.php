...
class Zwei_Admin_Elements_DojoFilteringSelect extends Zwei_Admin_Elements_Element
{
    function edit($i, $j, $display="block")
    {
        ...
        $return =  "<select dojoType=\"dijit.form.ComboBox\" id=\"edit{$i}_{$j}\" name=\"$this->target[$i]\" onload=\"dijit.byId('edit{$i}_{$j}').set('value', dijit.byId('edit{$i}_{$j}').get('value'))\" $value $onchange $required $regexp $invalid_message $prompt_message style=\"display:$display\" >\r\n$options\r\n</select>\r\n
        ";
        return $return;
    }
