...
class Zwei_Admin_Elements_DojoCheckedMultiselect extends Zwei_Admin_Elements_Element
{
	function edit($i, $j, $display="block")
	{
		...
		$return =  "<select dojoType=\"dojox.form.CheckedMultiSelect\" multiple=\"true\" id=\"edit{$i}_{$j}\" name=\"$this->target[$i]\" onload=\"dijit.byId('edit{$i}_{$j}').set('value', dijit.byId('edit{$i}_{$j}').get('value'))\" $value $onchange $required $regexp $invalid_message $prompt_message style=\"display:$display\" >\r\n$options\r\n</select>\r\n
		";
		return $return;
	}
    ...
    function options() 
    {
        ...
        if (!empty($this->params['TABLE'])) {
			...
			foreach ($rows as $row) {
			    Debug::write($row->{$id});
                $selected = in_array($row->{$id}, $this->value) ? "selected" : "";
                if (!empty($this->params['TABLE_FIELD'])) {
