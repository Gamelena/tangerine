 ...
 * 
 *     <helpers>
        <excel formatter="csv" name="Descargar CSV" iconClass="gamelenaIconDownload"/>
        <uploader name="Cargar CSV" action="load" truncate="true" iconClass="gamelenaIconUpload"></uploader>
    </helpers>
 * 
 * 
 * </code>
 ...
 */
class Gamelena_Utils_File_Uploader
{
    /**
     * 
     ...
     * @param Gamelena_Utils_Form $form
     */
    ...
    {
        $ext = Gamelena_Utils_File::getExtension($file['name']);
        ...
        if ($file['size'] == 0) {
            ...
            $separator = Gamelena_Utils_File::getSeparator($filename, 5);
                ...
                if ($validateLine) {
                    if (!empty($line[0])) {
                        ...
                        foreach ($this->_columns as $j => $column) {
                            $text = trim(Gamelena_Utils_String::textify($line[$j]));
