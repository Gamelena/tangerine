<?php
/**
 * Crea un objeto Zend_Db_Select o Zend_Db_Table_Select según parámetros enviados por Gamelena_Utils_Form ($_REQUEST),
 * generalmente es instanciado por ObjectsController pero puede instanciado en cualquier lugar, 
 * por ejemplo para sobrecargar manualmente Gamelena_Db_Table::select() usando parámetros equivalentes.
 *
 * @example
 * <code>
 ...
 * $select = $db->select();
 * $object = new Gamelena_Db_Object($form, $select);
 * $select = $object->select();  
 * </code>
 * @category Gamelena
 * @package  Gamelena_Db
...
class Gamelena_Db_Object
{
    ...
    /**
     * 
     * @param Gamelena_Utils_Form | array
     * @param Gamelena_Db_Select
     */
    public function __construct($form, $select = false)
    {
        if ($form instanceof Gamelena_Utils_Form) {
            $this->_form = $form;
        } else { //en este punto $form debe ser array
            $this->_form = new Gamelena_Utils_Form($form);
        }
    ...
    public function select()
    {
        ...
        if ($this->_select instanceof Zend_Db_Select) {
            Gamelena_Utils_Debug::writeBySettings($this->_select->__toString(), 'query_log');
        }
        return $this->_select;
    }
    /**
     * Recarga objeto Zend_Db_Select con parametros generados en buscador de Tangerine
     * 
